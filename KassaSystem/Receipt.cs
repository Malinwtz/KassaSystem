...
{
    public List<SingleReceipt> ListOfSingleReceipts { get; set; } = new();

    public void AddToListOfSingleReceipts(Products product)
    {
        ListOfSingleReceipts.Add(new SingleReceipt(product.ProductID, product.ProductName,
    ...
    {
        decimal total = 0;
        foreach (var row in ListOfSingleReceipts) total += row.TotalPrice;
        return total;
    }
    ...
    {
        var result = ListOfSingleReceipts.FirstOrDefault(p => p.ProductID == product.ProductID);
    ...
    public void ChangeCountToProductsTextFile()
    {
        ...
        {
            int count;
            var result = ListOfSingleReceipts.FirstOrDefault(p => p.ProductID == row.ProductID);
